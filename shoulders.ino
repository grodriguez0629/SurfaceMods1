...
void setup() {
  ...
  leftStepper.setCurrentMilliamps36v8(8000);
  ...
  rightStepper.setCurrentMilliamps36v8(8000);
...
void loop() {
  ...
  if(cmd[0] == 'L') {
    if(cmd[1] == 'U' && posLeft < MAX_ANGLE) {
      ...
      moveShoulder(cmd[0], UP, cmd.substring(2).toInt());
    }
    else if(cmd[1] == 'D' && posLeft > MIN_ANGLE) {
      setDirection(DIR_PIN1, DOWN);
      moveShoulder(cmd[0], DOWN, cmd.substring(2).toInt());
    }
    else {
