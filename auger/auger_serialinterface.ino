...
void loop() {
  ...
  spdProf = Serial.readStringUntil(' ');
  dir = Serial.readString();
  dir.trim();
  Serial.println(spdProf + " " + dir);
  ...
  else if(spdProf == "HIGH") {
    spdProfile(1);
  }
  else if(spdProf == "OFF") {
    spdProfile(2);
  }
  else if(spdProf == "ESTOP") {
    spdProfile(3);
  ...
  setDirection(dir);
  Serial.println("Direction set: " + dir);
